...
// --- КОНФИГУРАЦИЯ ---
// Для безопасности ключ вынесен в отдельный файл config.php, который не загружается в репозиторий
$config = file_exists(__DIR__ . '/config.php') ? require __DIR__ . '/config.php' : ['secret_key' => 'YOUR_SECRET_KEY'];
$secretKey = $config['secret_key'] ?? 'YOUR_SECRET_KEY';
$ytDlpPath = 'yt-dlp';
// ---------------------
...
try {
    ...
    // Специфичная логика для TikTok (использование embed для стабильности)
    if ($platform === 'tiktok') {
        $videoId = null;
    ...
    // Для YouTube и других платформ выбираем лучшее видео+аудио в одном файле (best)
    // Если нужен конкретно mp4, можно добавить --merge-output-format mp4 (требует ffmpeg)
    $command = escapeshellarg($ytDlpPath) . 
               " --no-playlist " .
               " --format \"best\" " .
               " --no-check-certificate " .
               " --user-agent \"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36\" " .
               " -o " . escapeshellarg($filePath) . " " . 
               escapeshellarg($downloadUrl) . " 2>&1";
